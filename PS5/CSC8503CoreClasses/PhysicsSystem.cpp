...
void PhysicsSystem::ImpulseResolveCollision(GameObject& a, GameObject& b, CollisionDetection::ContactPoint& p) const {
    ...
    if (physA->UseResolve() && physB->UseResolve()) {
        ...
        Vector3 angVelocityA =
            Maths::Vector::Cross(physA->GetAngularVelocity(), relativeA);
        Vector3 angVelocityB =
            Maths::Vector::Cross(physB->GetAngularVelocity(), relativeB);
        ...
        float impulseForce = Maths::Vector::Dot(contactVelocity, p.normal);

        // now to work out the effect of inertia ....
        Vector3 inertiaA = Maths::Vector::Cross(physA->GetInertiaTensor() *
            Maths::Vector::Cross(relativeA, p.normal), relativeA);
        Vector3 inertiaB = Maths::Vector::Cross(physB->GetInertiaTensor() *
            Maths::Vector::Cross(relativeB, p.normal), relativeB);
        float angularEffect = Maths::Vector::Dot(inertiaA + inertiaB, p.normal);
        ...
        physA->ApplyAngularImpulse(Maths::Vector::Cross(relativeA, -fullImpulse));

        physB->ApplyAngularImpulse(Maths::Vector::Cross(relativeB, fullImpulse));
    }
}
...
void PhysicsSystem::NarrowPhase() {
    for (std::set<CollisionDetection::CollisionInfo>::iterator i = broadphaseCollisions.begin();
        ...
        if (CollisionDetection::ObjectIntersection(info.a, info.b, info)) {
            //if (info.a->GetTypeID() == 10010 && info.b->GetTypeID() == 10010) {
